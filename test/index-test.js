...
test('render() video  ', (t) => {
  ...
  const expected = renderToStaticMarkup(<video width='100' height='200'>
...
test('render() tweet - normal', (t) => {
  ...
  const expected = renderToStaticMarkup(<blockquote className='twitter-tweet' data-lang='en'>
...
test('render() tweet - embedAs defaults to tweet', (t) => {
  const input = {
    type: 'twitter',
    embedAs: 'unknown',
    text: [
...
test('render() twitter video', (t) => {
  const input = {
    type: 'twitter',
    embedAs: 'video',
    user: { name: 'Twitter Video', slug: 'video' },
    date: 'June 1, 2016',
    text:
     [{ content: 'text content', href: null }],
    id: '737840608895762432',
    url: 'https://twitter.com/video/status/737840608895762432',
  };
  const actual = render(input);
  const expected = fixtures.tweetVideo;
  t.is(actual, expected);
});
...
test('render() facebook - embedAs defaults to post', (t) => {
  const input = {
    url: 'https://www.facebook.com/david.bjorklund/posts/10153809692501070',
    type: 'facebook',
    embedAs: 'unknown',
...
test('render() tidal', (t) => {
  ...
  const actual = render(input);
  const expected = renderToStaticMarkup(<div className='tidal-embed' data-type='data-type' data-id='data-id' />);
  t.is(actual, expected);
});
